...
for fold in k_folds:
    ...
    while user_count < number_of_users:
        ...
        if len(fold.loc[fold['user_id'] == user_count + 1]) != 0:
            ...
            selection = ((fold.loc[fold['user_id'] == user_count + 1]).sort_values(by='rating', ascending=0)).iloc[0]
    ...
    weight_matrix = LapLRSFunc(np.asmatrix(ratings_training_set), ratings_test_set)
    print("********************** End of fold *************************")
